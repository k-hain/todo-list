...
// Fallback for modularized imports:
/* harmony default export */ const date_fns_setMilliseconds = ((/* unused pure expression or super */ null && (setMilliseconds)));
...
const preventNewLineOnEnter = (el) => {
...
const addHeaderBlurEvent = (el, publishFunc, listIndex, taskIndex) => {
...
;// ./src/svg/add_24dp_000000_FILL0_wght400_GRAD0_opsz24.svg
const add_24dp_000000_FILL0_wght400_GRAD0_opsz24_namespaceObject = __webpack_require__.p + "db8059549b1bd98b4242.svg";
;// ./src/svg/circle_16dp_000000_FILL0_wght400_GRAD0_opsz20.svg
const circle_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject = __webpack_require__.p + "62654d1ac47e565dda59.svg";
;// ./src/svg/task_alt_16dp_000000_FILL0_wght400_GRAD0_opsz20.svg
const task_alt_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject = __webpack_require__.p + "29857105507087048be2.svg";
;// ./src/svg/calendar_month_16dp_000000_FILL0_wght400_GRAD0_opsz20.svg
const calendar_month_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject = __webpack_require__.p + "534192205c718b77cfe7.svg";
;// ./src/svg/delete_forever_16dp_000000_FILL0_wght400_GRAD0_opsz20.svg
const delete_forever_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject = __webpack_require__.p + "fa54c53e60940639f29e.svg";
;// ./src/svg/label_16dp_000000_FILL0_wght400_GRAD0_opsz20.svg
const label_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject = __webpack_require__.p + "21753647b78cc906d92e.svg";
;// ./src/svg/done_all_16dp_000000_FILL0_wght400_GRAD0_opsz20.svg
const done_all_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject = __webpack_require__.p + "aa33f65a94a67dc83d57.svg";
;// ./src/index.js
...
const displayController = (function () {
  ...
  const initializePage = (msg) => {
    drawSidebar();
    pubsub_default().publish('LOAD_DATA');
  };
  const initializePageToken = 
    pubsub_default().subscribe('INITIALIZE_PAGE', initializePage);
  ...
  const drawSidebarHeader = (container) => {
    ...
    drawImgElement(add_24dp_000000_FILL0_wght400_GRAD0_opsz24_namespaceObject, addListBtnEl);
    ...
    addListBtnEl.addEventListener('click', () => {
      ...
      while (keepGoing) {
        ...
        if (newName) {
          pubsub_default().publish('NEW_LIST', newName);
          keepGoing = false;
  ...
  const drawSidebarButtons = (msg, listNames) => {
    ...
    listNames.forEach((listName) => {
      ...
      listNameEl.addEventListener('click', () => {
        ...
        listNameEl.classList.add('selected');
        pubsub_default().publish('SEND_LIST_TO_DRAW', listNames.indexOf(listName));
      });
    });
  };
  const drawSidebarButtonsToken =
    pubsub_default().subscribe('UPDATE_LIST_NAMES', drawSidebarButtons);

  const pubChangeListName = (listIndex, newName) => {
    pubsub_default().publish(
  ...
  const drawListToken = pubsub_default().subscribe('DRAW_LIST_PAGE', drawListPage);
  ...
  const drawTaskContainer = (tasksArr, container, listIndex) => {
    ...
    if (tasksArr.length) {
      ...
      sortable_esm.create(taskContainerEl, taskContainerOptions);
      pubsub_default().publish(
        'DRAW_TASK_DETAILS',
  ...
  const drawTask = (task, container, taskIndex, listIndex) => {
    ...
    taskEl.addEventListener('click', () => {
      ...
      taskEl.classList.add('task-highlighted');
      pubsub_default().publish('SEND_TASK_TO_DRAW', [listIndex, taskIndex]);
    });
  };
  ...
  const drawTaskDoneButton = (container, task, taskIndex, listIndex) => {
    ...
    taskButtonDoneEl.addEventListener('click', () => {
      pubsub_default().publish('CHANGE_TASK_ISFINISHED', [listIndex, taskIndex]);
    });

    const taskIconEl = drawImgElement(
      circle_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject, taskButtonDoneEl, ['task-icon']
    );
    const taskIconHoverEl = drawImgElement(
      task_alt_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject, taskButtonContainerEl, ['task-icon-hover']
    );
    if (task.isFinished) {
      taskIconHoverEl.src = circle_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject;
      taskIconEl.src = task_alt_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject;
    }
  ...
  const drawTaskDate = (container, task, taskIndex, listIndex) => {
    ...
    drawImgElement(calendar_month_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject, taskDueButtonEl, ['icon-due']);
    ...
    taskDuePickerEl.addEventListener('input', (evt) => {
      ...
      pubsub_default().publish('CHANGE_TASK_DATE', [
  ...
  const drawNewTaskForm = (indexVal, container) => {
    ...
    drawImgElement(add_24dp_000000_FILL0_wght400_GRAD0_opsz24_namespaceObject, addTaskBtnEl);
  
    addTaskFormEl.addEventListener('submit', (event) => {
      event.preventDefault();
      if (addTaskInputEl.value) {
        pubsub_default().publish('NEW_TASK', [addTaskInputEl.value, indexVal]);
      }
    });
  };
  ...
  const drawPriorityDropdown = (container, task, taskIndex, listIndex) => {
    ...
    drawImgElement(
      label_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject,
      dropdownButtonEl,
    ...
    for (let i = 0; i < 3; i++) {
      ...
      drawImgElement(
        label_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject, dropdownEntryEl, [`priority-${i}`, 'icon-priority']
      );
      drawDomElement('span', dropdownEntryEl, [], getPriorityName(i));
      dropdownEntryEl.addEventListener('click', () => {
        pubsub_default().publish('CHANGE_TASK_PRIORITY', [listIndex, taskIndex, i]);
      });
    }
  ...
  const drawTaskDetails = (msg, [task, taskIndex, listIndex]) => {
    ...
    if (task.isFinished) {
      drawImgElement(task_alt_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject, markTaskButtonEl, ['icon-checked']);
      drawDomElement('span', markTaskButtonEl, [], 'Completed');
      markTaskButtonEl.classList.add('completed');
    } else {
      drawImgElement(done_all_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject, markTaskButtonEl, ['icon-done']);
    ...
    drawImgElement(delete_forever_16dp_000000_FILL0_wght400_GRAD0_opsz20_namespaceObject, deleteTaskButtonEl, ['icon-delete']);
    ...
    deleteTaskButtonEl.addEventListener('click', () => {
      if(confirm(`Delete ${task.name}?`)) {
        ...
        pubsub_default().publish('DELETE_TASK', [listIndex, taskIndex]);
      }
    });
  ...
  const drawTaskDetailsToken =
    pubsub_default().subscribe('DRAW_TASK_DETAILS', drawTaskDetails);
})();

pubsub_default().publish('INITIALIZE_PAGE');
})();
